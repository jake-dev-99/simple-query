...
public class SimpleQueryIosPlugin: NSObject, FlutterPlugin, NativeQueryHostApi {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let instance = SimpleQueryIosPlugin(binaryMessenger: registrar.messenger())
    ...
    return [
      "capabilities": [
        ...
        capability(domain: "messages", read: false, write: false, observe: false, stream: false,
                   reason: "simple_query: messages is not supported on iOS"),
        capability(domain: "calls", read: false, write: false, observe: false, stream: false,
                   reason: "simple_query: calls is not supported on iOS"),
  ...
  public func query(request: [String?: Any?]) throws -> [String?: Any?] {
    ...
    default:
      throw notSupported("simple_query: query is not supported for domain \(domain) on iOS host")
    }
  ...
  public func mutate(request: [String?: Any?]) throws -> [String?: Any?] {
    ...
    guard domain == "files" || domain == "media" else {
      throw notSupported("simple_query: mutate is not supported for domain \(domain) on iOS host")
    }
  ...
  public func observeStart(request: [String?: Any?]) throws -> String {
    ...
    guard domain == "files" || domain == "media" || domain == "contacts" || domain == "calendar" else {
      throw notSupported("simple_query: observe is not supported for domain \(domain) on iOS host")
    }
  ...
  public func openBinary(request: [String?: Any?]) throws -> [String?: Any?] {
    let domain = (request["domain"] as? String) ?? "platformSpecific"
    guard domain == "files" || domain == "media" else {
      throw notSupported("simple_query: stream is not supported for domain \(domain) on iOS host")
    }
  ...
  public func callExtension(
    ...
    case "ios.photos":
      if method == "fetchAssetResources" {
        ...
        if let limitValue, !(limitValue is Int) {
          throw invalidQuery("simple_query: ios.photos.fetchAssetResources expects limit as Int")
        }
        let rootPathValue = args?["rootPath"]
        if let rootPathValue, !(rootPathValue is String) {
          throw invalidQuery("simple_query: ios.photos.fetchAssetResources expects rootPath as String")
        }
      ...
      if method == "listMediaTypes" {
        if args != nil, !(args?.isEmpty ?? true) {
          throw invalidQuery("simple_query: ios.photos.listMediaTypes does not accept arguments")
        }
        return [
    ...
    case "ios.contacts":
      if method == "listContainers" {
        if args != nil, !(args?.isEmpty ?? true) {
          throw invalidQuery("simple_query: ios.contacts.listContainers does not accept arguments")
        }
        return [
          "containers": try listContactContainers(),
          "authorizationStatus": contactsAuthorizationStatusName(),
        ]
      }
      if method == "listGroups" {
        if args != nil, !(args?.isEmpty ?? true) {
          throw invalidQuery("simple_query: ios.contacts.listGroups does not accept arguments")
        }
        return [
      ...
      if method == "listUnifiedSources" {
        if args != nil, !(args?.isEmpty ?? true) {
          throw invalidQuery("simple_query: ios.contacts.listUnifiedSources does not accept arguments")
        }
        return [
      ...
      if method == "requestScopedAccess" {
        let scopeValue = args?["scope"]
        if let scopeValue, !(scopeValue is String) {
          throw invalidQuery("simple_query: ios.contacts.requestScopedAccess expects scope as String")
        }
        let access = contactsReadAccess()
        return [
          "granted": access.readable,
          "status": contactsAuthorizationStatusName(),
    ...
    case "ios.calendar":
      if method == "listCalendars" {
        if args != nil, !(args?.isEmpty ?? true) {
          throw invalidQuery("simple_query: ios.calendar.listCalendars does not accept arguments")
        }
        return [
      ...
      if method == "getDefaultTimeZone" {
        if args != nil, !(args?.isEmpty ?? true) {
          throw invalidQuery("simple_query: ios.calendar.getDefaultTimeZone does not accept arguments")
        }
        return [
    ...
    throw notSupported("simple_query: \(namespace).\(method) is not supported on iOS host")
  }
  ...
  private func contactsReadAccess() -> (readable: Bool, reason: String?) {
    ...
    case .authorized:
      return (true, nil)
    case .notDetermined:
      return (false, "simple_query: contacts permission has not been granted on iOS")
    case .restricted, .denied:
      return (false, "simple_query: contacts access is denied on iOS")
    @unknown default:
      return (false, "simple_query: contacts access is unavailable on iOS")
    }
  }
  ...
  private func calendarReadAccess() -> (readable: Bool, reason: String?) {
    if #available(iOS 17.0, *) {
      ...
      case .fullAccess:
        return (true, nil)
      case .writeOnly:
        return (false, "simple_query: calendar read access is unavailable (write-only) on iOS")
      case .notDetermined:
        return (false, "simple_query: calendar permission has not been granted on iOS")
      case .restricted, .denied:
        return (false, "simple_query: calendar access is denied on iOS")
      @unknown default:
        return (false, "simple_query: calendar access is unavailable on iOS")
      }
    }
